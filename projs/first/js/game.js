...
function cellClicked(ev) {
    ...
    if (gGame.isOn === true && choosenCell.isMine === true && ev.button !== 2) {
        if (isHints === true) {
            hintsProcess(location.i, location.j);
        } else {
        exposeAllMines()
        victory = false;
...
// function hintsInsert() {
//     var hint = `<button class="hint hint1" onclick="hints()">&#128269;</button>
//     <button class="hint hint2" onclick="hints()">&#128269;</button>
//     <button class="hint hint3" onclick="hints()">&#128269;</button>`;
//     var hintBox = document.querySelector('.hints')
//     hintBox.innerHTML = `<h3>You got hints:</h3> ${hint}</div>`;

// }
function hintsremove() {
    console.log();
    hintsleft.pop();
}
function hints() {
    hintsCounter++
    if (hintsCounter === 3) {
        var removeHint = document.querySelector('.hint1');
        removeHint.classList.add('hidden');
        isHints = false;
    }
    isHints = true;
}
function hintsProcess(i, j) {
    // console.log(i, j);
    // debugger;
    var hintsArr = [];
    var x = +i - 1;
    ...
    for (x; x <= +i + 1; x++) {
        for (y; y <= +j + 1; y++) {
            console.log(x, y);
            // debugger;
            // if (gBoard[x][y] < 0) continue;
            // else {
                // if (gBoard[x][y].isShown === true) continue;
                // gBoard[x][y].isShown = true;
                var location = {
                    i: x,
                    j: y
                }
                hintsArr.push(location);
